        ...
        return (
            <div className='absolute insert-0 bg-black text-white min-h-screen'>
                ...
                {place?.photos?.length > 0 && place.photos.map(photo => (
                    <div>
                        <img src={'https://booking-app-api1.onrender.com/uploads/'+photo} alt={place.title} />
                    </div>
                ))}
                </div>
                </div>
        ...
        <div className='mt-4 bg-gray-100 -mx-8 px-8 py-8'>
            ...
            <div className='grid gap-2 grid-cols-[2fr_1fr] rounded-3xl overflow-hidden'>
                <div>
                    ...
                    <img onClick={() => setShowAllPhotos(true)} className='object-cover cursor-pointer aspect-square w-full h-full' src={'https://booking-app-api1.onrender.com/uploads/'+place.photos[0]} alt={place.title} />
                </div>
                    )}
            </div>
            <div className='grid'>
                {place.photos?.[1]  && (
                    <img onClick={() => setShowAllPhotos(true)} className='object-cover cursor-pointer aspect-square' src={'https://booking-app-api1.onrender.com/uploads/'+place.photos[1]} alt={place.title} />
                )} 
                <div className='overflow-hidden'>
                    {place.photos?.[2]  && (
                        <img onClick={() => setShowAllPhotos(true)} className='object-cover cursor-pointer aspect-square' src={'https://booking-app-api1.onrender.com/uploads/'+place.photos[2]} alt={place.title} />
                    )}
                </div>
            </div>
            </div>
